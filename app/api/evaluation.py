...
async def quick_evaluate(request: QuickEvaluationRequest):
    ...
    llm = _build_dashscope_llm()
    embeddings = _build_dashscope_embeddings()
    ...
    async def _compute_one_metric(metric_name, metric_cls, metric_inputs, metric_kwargs=None):
        """后端 在独立线程中计算单个指标，避免 asyncio.run() 事件循环冲突"""
        def _do():
            kwargs = metric_kwargs or {}
            m = metric_cls(**kwargs)
            s = m.batch_score(metric_inputs)  # 后端 RAGAS batch_score 内部用 asyncio.run()
            return metric_name, round(float(sum(x.value for x in s) / len(s)), 4)
        try:
            return await asyncio.to_thread(_do)  # 后端 不阻塞主事件循环
        except Exception as e:
            return metric_name, f"计算失败: {e}"

    results = await asyncio.gather(
        _compute_one_metric("faithfulness", Faithfulness,
            faithfulness_inputs, {"llm": llm}),
        _compute_one_metric("answer_relevancy", AnswerRelevancy,
            answer_relevancy_inputs, {"llm": llm, "embeddings": embeddings}),
        _compute_one_metric("context_precision", ContextPrecision,
            context_precision_inputs, {"llm": llm}),
    )  # 后端 三个指标并行跑，不阻塞事件循环
...
async def evaluate_answer(request: AnswerEvalRequest):
    ...
    流程：检索上下文 → 异步并行计算三指标 → 返回分数（不阻塞事件循环，评估期间可正常聊天）
    """
    ...
    # 并行计算三个指标（asyncio.gather，不阻塞事件循环）
    llm = _build_dashscope_llm()
    embeddings = _build_dashscope_embeddings()

    async def _compute_one(metric_name, metric_cls, metric_inputs, metric_kwargs=None):
        """后端 在独立线程中计算单个指标"""
        def _do():
            kwargs = metric_kwargs or {}
            m = metric_cls(**kwargs)
            s = m.batch_score(metric_inputs)
            return metric_name, round(float(sum(x.value for x in s) / len(s)), 4)
        try:
            return await asyncio.to_thread(_do)  # 后端 不阻塞主事件循环
        except Exception as e:
